...
class Character3:
    def __init__(self, x, y, character_image_path, bullet_image_path):
        ...
        self.max_jumps = 2  # 허용된 최대 점프 횟수
        self.remaining_jumps = self.max_jumps  # 남은 점프 횟수
    ...
    def move(self, joystick, platforms, monsters):
        ...
        if joystick.is_button_pressed(joystick.button_U) and not self.jumping and on_platform:
            self.jumping = True
            self.jump_frame_count = 0
            self.remaining_jumps -= 1  # 남은 점프 횟수 감소
            self.additional_jump = False  # 추가 점프 여부 초기화
        
        # B 버튼 확인 및 추가 점프
        if joystick.is_button_pressed(joystick.button_B) and (self.remaining_jumps > 0 or self.additional_jump):
            self.jumping = True
            self.jump_frame_count = 0
            self.remaining_jumps -= 1  # 남은 점프 횟수 감소
            self.additional_jump = False  # 추가 점프 여부 초기화
